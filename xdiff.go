...
func version() {
	fmt.Printf("xdiff. Version %s\n", VERSION)
}

func XdiffToString(file1, file2 string) (error, string) {
	...
	if err1 != nil || err2 != nil {
		...
		return fmt.Errorf("file1[%s] or file2[%s] stat error.", file1, file2), ""
	}

	if finfo1.IsDir() != finfo2.IsDir() {
		return fmt.Errorf("Unable to compare file and directory"), ""
	}

	outF := new(bytes.Buffer)
	out = bufio.NewWriterSize(outF, OUTPUT_BUF_SIZE)
	...
	if !flag_output_as_text {
		...
		fmt.Fprintf(out, "<p>XDIFF version: %s<hr>=====>&nbsp;<strong><font color=\"#e68a00\">%s</font></strong><br> DIFF <br>=====>&nbsp;<strong><font color=\"#e68a00\">%s</font></strong></p>\n", VERSION, html.EscapeString(file1), html.EscapeString(file2))
	}
	...
	return nil, outF.String()
}

func Xdiff(file1, file2, output string) error {
	if flag_cmp_ignore_case || flag_cmp_ignore_space_change || flag_cmp_ignore_all_space {
		if flag_unicode_case_and_space {
			compute_hash = compute_hash_unicode
			compare_line = compare_line_unicode
	...
	if !flag_output_as_text {
		...
		out.WriteString(HTML_CSS)
		out.WriteString("</head><body>\n")
		fmt.Fprintf(out, "<p>XDIFF version: %s<hr>=====>&nbsp;<strong>%s</strong><br> DIFF <br>=====>&nbsp;<strong>%s</strong></p>\n", VERSION, html.EscapeString(file1), html.EscapeString(file2))
	}
